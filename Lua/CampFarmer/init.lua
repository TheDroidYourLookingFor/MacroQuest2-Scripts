...
local CampFarmer = { _version = '1.0.0', _author = 'TheDroidUrLookingFor' }
...
CampFarmer.Settings.DoLoot = false
...
CampFarmer.Settings.lootINIFile = 'EZLoot\\EZLoot-WT.ini'
CampFarmer.Settings.MinMobsInZone = 10
CampFarmer.Settings.UberPullMobsInZone = 100
CampFarmer.Settings.ReturnToHomeDistance = 10
...
function CampFarmer.ClearCorpses()
...
function CampFarmer.CheckCorpseCount()
    ...
    if not CampFarmer.Settings.DoLoot and mq.TLO.SpawnCount('corpse')() >= 50 then CampFarmer.ClearCorpses() end
end
...
function CampFarmer.KillThis()
    ...
    if mq.TLO.Pet() and not mq.TLO.Pet.Combat() then mq.cmd('/squelch /pet attack') end
end
...
function CampFarmer.CheckBuffs()
    ...
    if CampFarmer.Settings.useCoinSack and mq.TLO.Me.ItemReady('Bemvaras\' Coin Sack')() then
        ...
        mq.delay(5000, function () return mq.TLO.Me.Casting.ID() == 0 end)
    ...
    if CampFarmer.Settings.useClericAA and not mq.TLO.Me.Buff('Cleric Mastery - Divine Health')() and mq.TLO.Me.AltAbilityReady(CampFarmer.ClassAAs['Cleric'])() and mq.TLO.SpawnCount(CampFarmer.Settings.spawnSearch)() == 0 then
        if not mq.TLO.Me.Casting() and not mq.TLO.Me.Combat() then
            ...
            mq.delay(2500, function() return mq.TLO.Me.Buff('Cleric Mastery - Divine Health').ID() > 0 end)
...
function CampFarmer.RespawnZone()
    ...
    if not mq.TLO.FindItem(CampFarmer.Settings.respawnItem)() then return end
    ...
    if mq.TLO.SpawnCount('npccorpse')() > 50 and not CampFarmer.Settings.DoLoot then CampFarmer.ClearCorpses() end
...
function CampFarmer.Aggro(aggroCharm)
    if CampFarmer.CheckXTargAggro() > 0 then return end
    mq.cmdf('/target id %s', mq.TLO.Me.ID())
    mq.delay(1000, function() return mq.TLO.Target.ID() == mq.TLO.Me.ID() end)
    mq.delay(CampFarmer.FastDelay)
    mq.cmdf('/useitem %s', aggroCharm)
    mq.delay(CampFarmer.delay.AggroDelay)
end
function CampFarmer.AggroZone()
    ...
    if mq.TLO.SpawnCount(CampFarmer.Settings.mobsSearch)() < CampFarmer.Settings.MinMobsInZone then return end
    if not mq.TLO.FindItem(CampFarmer.Settings.aggroItem)() then return end
    if not mq.TLO.Me.ItemReady(CampFarmer.Settings.aggroItem)() then return end
    if mq.TLO.NearestSpawn(CampFarmer.Settings.spawnSearch)() then return end
    if CampFarmer.CheckXTargAggro() > 0 then return end
...
function CampFarmer.CheckTarget()
    ...
    if not mq.TLO.Me.Standing() then mq.TLO.Me.Stand() end
    -- print(CampFarmer.CheckDistance(mq.TLO.Me.X(), mq.TLO.Me.Y(), mq.TLO.Me.Z()))
    if CampFarmer.CheckDistance(mq.TLO.Me.X(), mq.TLO.Me.Y(), mq.TLO.Me.Z()) > CampFarmer.Settings.ReturnToHomeDistance then
        if (mq.TLO.Target() and not string.find(mq.TLO.Target.CleanName(), 'Treasure Goblin')) or not mq.TLO.Target() then
            mq.cmdf('/warp loc %s %s %s', CampFarmer.startY, CampFarmer.startX, CampFarmer.startZ)
        end
    end
    ...
    if not mq.TLO.Me.Combat() and mq.TLO.Target() then CampFarmer.KillThis() end
...
function CampFarmer.Main()
    ...
    if mq.TLO.Pet.ID() then CampFarmer.CheckPetAoE() end
    CampFarmer.CheckBuffs()
